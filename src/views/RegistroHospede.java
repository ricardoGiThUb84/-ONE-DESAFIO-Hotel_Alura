...
public class RegistroHospede extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					RegistroHospede frame = new RegistroHospede(reserva);
	...
	public RegistroHospede(Reserva reserva) {
		...
		txtNreserva.setText(reserva == null ? "" : String.valueOf(reserva.getId()));
		...
		btnsalvar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				boolean nome = txtNome.getText().equalsIgnoreCase("");
				boolean telefone  = txtTelefone.getText().equalsIgnoreCase("");
				boolean dataNascimento =  ((JTextField) txtDataN.getDateEditor()
						.getUiComponent()).getText().equalsIgnoreCase("");
		    	
				if (!(nome || telefone || dataNascimento)) {
					
					if(!ValidadorCampoHospede.validarTelefone(txtTelefone.getText())){
						
						JOptionPane.showMessageDialog(null,
								"Digite 11 dígitos no campo telefone! Ex: 71976778456.");
	...
	//Código que permite movimentar a janela pela tela seguindo a posição de "x" y "y"
	 private void headerMousePressed(java.awt.event.MouseEvent evt) {
	    ...
	    public void salvarHospede() {
	    	...
	    	String reserva = txtNreserva.getText();
	    	...
	    	HospedeController hospedeDAO = new HospedeController();
