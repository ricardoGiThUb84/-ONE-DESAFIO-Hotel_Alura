...
import Util.CalculaDiferencaDataUtil;
import Util.baseCalculoDesconto.BaseCalculo;
import Util.baseCalculoDesconto.BaseCalculoCincoPorcento;
import Util.baseCalculoDesconto.BaseCalculoDezPorcento;
import Util.baseCalculoDesconto.BaseCalculoPadrao;
...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		txtDataS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				...
				try {
					...
					txtValor.setText(calcularValorReserva());
					
				} catch (Exception e) {
					
					// TODO Auto-generated catch block
					
					if(((JTextField) txtDataE.getDateEditor().getUiComponent()).getText().equalsIgnoreCase("") 
					&& ((JTextField) txtDataS.getDateEditor().getUiComponent()).getText().equalsIgnoreCase("") ) {
						
					}else {
						
						mensagemDataInvalida(e);
		...
		//Ativa o evento, após o usuário selecionar as datas, o valor da reserva deve ser calculado
			...
			private void mensagemDataInvalida(Exception e) {
				JOptionPane.showMessageDialog(null, e.getMessage());
				((JTextField) txtDataE.getDateEditor().getUiComponent()).setText("");
				((JTextField) txtDataS.getDateEditor().getUiComponent()).setText("");
				txtValor.setText("");
			}
		});
		...
		txtValor.setBounds(15, 328, 400, 33);
	...
	//Código que permite movimentar a janela pela tela seguindo a posição de "x" e "y"	
	 private void headerMousePressed(java.awt.event.MouseEvent evt) {
	    ...
	    private void salvarRegistro() {
	    	...
	    	String tipoPagamento = txtFormaPagamento.getSelectedItem().toString();
	    	
	    	Reserva reserva = new Reserva(Date.valueOf(dataEntrada),
	    			Date.valueOf(dataSaida),extrairValor(txtValor.getText()), tipoPagamento);

	    	
	    	reservaController.salvar(reserva);
	    	
	    	JOptionPane.showMessageDialog(contentPane,"Registro salvo com reserva: " + reserva.getId());
	    ...
	    private String extrairValor(String text) {
		...
		private String calcularValorReserva() throws Exception {
	    	
	    	
	    	    String dataEntrada = ((JTextField) txtDataE.getDateEditor().getUiComponent()).getText();
		    	String dataSaida = ((JTextField) txtDataS.getDateEditor().getUiComponent()).getText();
		    	
		    	BaseCalculo calculoReserva = new BaseCalculoDezPorcento(
		    			new BaseCalculoCincoPorcento(new BaseCalculoPadrao(null)));
	    		
	    		
	    		
					return calculoReserva.calcular(CalculaDiferencaDataUtil
							.calculoDias(Date.valueOf(dataEntrada), Date.valueOf(dataSaida)));
		 }
}
